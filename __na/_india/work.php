<?php include($_SERVER['DOCUMENT_ROOT'].'/_inc/header.php') ?>
	<div id="page">
		<div id="inner">
		

	<div id="work">
	<!-- intro -->
		<div id="intro">
			<h2>Over the past 5 years we have worked on all manner of digital projects including <em>websites</em>, <em>microsites</em>, <em>facebook</em> and <em>mobile apps</em>, as well as <em>banners</em> and <em>html emails</em>.

				As we <em>white label</em> for many of our clients we are unable to talk about much of the great work we do, so if you would like to see more please <a href="/contact.php">get in touch</a>.</h2>
	<?php include($_SERVER['DOCUMENT_ROOT'].'/_inc/fb.php') ?>		</div>
		<!-- slider -->
			<?php include($_SERVER['DOCUMENT_ROOT'].'/_inc/slideshow.php') ?>	
	<!--  4 cols -->
		<div id="threeCol">
		
		<div class="row">
						<div>
						<a class="fancybox fancybox.ajax" href="/_work/w-sky.html" title="We designed and developed this CMS for Sky to house and share their brand assets and showcase past campaigns"><h3>CMS Website -Sky</h3>
						<img src="/_work/_img/_thumb/w-sky.jpg" alt="o-moo"/></a>
					</div>
					<div>
						<a class="fancybox fancybox.ajax" href="/_work/r-mazda.html" title="We designed and built this simple yet engaging driving game for Mazda2"><h3>Rich Media - Mazda 2</h3>
						<img src="/_work/_img/_thumb/r-mazda.jpg" alt="o-microsoft"/></a>
					</div>
					<div>
						<a class="fancybox fancybox.ajax" href="/_work/r-airNz.html" title="Working from scamps and assets we brought this ATL campaign to life online"><h3>Rich media - Air New Zealand</h3>
						<img src="/_work/_img/_thumb/r-airnz.jpg" alt="s-microsoft"/></a>

					</div>
			</div>
			<div class="row">
					<div>
						<a class="fancybox fancybox.ajax" href="/_work/f-mazda.html" title="From the success of the game we designed, we were asked to create a Facebook app where users could build own tracks and race against their friends "><h3>Facebook App - Mazda Connect</h3>
						<img src="/_work/_img/_thumb/f-mazda.jpg" alt="Mazda"/></a>
					</div>
					<div>
						<a class="fancybox fancybox.ajax" href="/_work/f-iceWatch.html" title="We developed this personality quiz and prize draw app"><h3>Facebook App - Ice Watch</h3>
						<img src="/_work/_img/_thumb/f-iceWatch.jpg" alt="Ice Watch "/></a>
					</div>
				<div>
						<a class="fancybox fancybox.ajax" href="/_work/o-betfair.html" title="Approximately 150 resizes in 13 different languages and a very tight deadline"><h3>Standard Banners- Betfair</h3>
						<img src="/_work/_img/_thumb/o-betfair.jpg" alt="e-betfair"/></a>
					</div>
			</div>
			
		<div class="row">
		<div>
						<a class="fancybox fancybox.ajax" href="/_work/e-bertolli.html" title="We created a master template for this series of emails allowing for efficient creation of variants and updates"><h3>HTML email - Bertolli</h3>
						<img src="/_work/_img/_thumb/e-bertolli.jpg" alt="Bertolli"/></a>
					</div>
					<div>
						<a class="fancybox fancybox.ajax" href="/_work/w-renault.html" title="For Renault's on-going competition microsites we created templates for them to be updated easily and cost-effectively"><h3>Competition microsite - Renault</h3>
						<img src="/_work/_img/_thumb/w-renault.jpg" alt="r-airNz"/></a>
					</div>
					
					<div>
						<a class="fancybox fancybox.ajax" href="/_work/w-inossa.html" title="We designed and developed this wordpress site allowing our client to easily manage and update it"><h3>Wordpress website - Inossa</h3>
						<img src="/_work/_img/_thumb/w-inossa.jpg" alt="w-ellas"/></a>
					</div>
			
					
			</div>
			<div class="row">
					<div class="logos">
					<h3>Some of the great brands we've worked on</h3>
					<img src="/_img/logos/Argos_logo.jpg" alt="Argos" />
					<img src="/_img/logos/t_mobile.jpg" alt="T Mobile" />
					<img src="/_img/logos/JP_Morgan.jpg" alt="JP Morgan" />
					<img src="/_img/logos/Douwe_Egberts_logo.jpg" alt="Douwe Egberts" />
					<img src="/_img/logos/Duracell_logo.jpg" alt="Duracell" />
					<img src="/_img/logos/Sky_logo.jpg" alt="Sky" />
					<img src="/_img/logos/Colgate_logo.jpg" alt="Colgate" />
					<img src="/_img/logos/boots.jpg" alt="Boots" />
					<img src="/_img/logos/aa.jpg" alt="AA" />
					<img src="/_img/logos/Marks_and_Spencer.jpg" alt="Marks and Spencer" />
					<img src="/_img/logos/three.jpg" alt="three" />
					<img src="/_img/logos/betfair.jpg" alt="Betfair" />
					<img src="/_img/logos/Orange.jpg" alt="Orange" />
					<img src="/_img/logos/air_new_zealand.jpg" alt="Air New Zealand" />
					<img src="/_img/logos/virgin_media.jpg" alt="Virgin Media" />
					<img src="/_img/logos/Vodafone_logo.jpg" alt="Vodafone" />
					<img src="/_img/logos/Accenture.jpg" alt="Accenture" />
					<img src="/_img/logos/blackberry.jpg" alt="Blackberry" />
					<img src="/_img/logos/Bensons.jpg" alt="Bensons" />
					<img src="/_img/logos/htc.jpg" alt="HTC" />
					<img src="/_img/logos/Microsoft_logo.jpg" alt="Microsoft" />
					<img src="/_img/logos/m_tv.jpg" alt="MTV" />
					<img src="/_img/logos/Auto_Trader.jpg" alt="Auto_Trader" />
					<img src="/_img/logos/bp.jpg" alt="bp" />
					<img src="/_img/logos/Direct_line.jpg" alt="Direct_line" />
					<img src="/_img/logos/doveSpa.jpg" alt="doveSpa" />
					<img src="/_img/logos/Hellmanns.jpg" alt="Hellmanns" />
					<img src="/_img/logos/mazda.jpg" alt="mazda" />
					<img src="/_img/logos/national-lottery.jpg" alt="National Lottery" width="144" height="150" />
					<img src="/_img/logos/Nokia_logo.jpg" alt="Nokia" />
					<img src="/_img/logos/sony.jpg" alt="Sony" />
					<img src="/_img/logos/which.jpg" alt="Which" />
					</div>
		</div>		
		
		</div>
	</div>			
</div>				
</div>
<?php include($_SERVER['DOCUMENT_ROOT'].'/_inc/footer.php') ?>
